...
<body class="p-3 m-1 border-1 bd-example">
    <div class="container"> <!--Crear controlador para esta vista-->
        ...
        <table class="table">
            ...
            <tbody>
                @forelse($adminlistadomantenimiento as $alm)
                    <tr>
                        <td>{{ $alm->id }}</td>
                        <td>{{ $alm->rut_prev }}</td>
                        <td>{{ $alm->nombre_prev }}</td>
                        <td>{{ $alm->apellidos_prev }}</td>
                        <td>{{ $alm->rut_cap }}</td>
                        <td>{{ $alm->nombre_cap }}</td>
                        <td>{{ $alm->apellidos_cap }}</td>
                        <td>{{ $alm->rol_cap }}</td>
                        <td> <a class="btn btn-danger" href="/Archivos/mantenimiento/{{ $alm->documentos }}" target="blank_">Ver Documentos</a> </td>
                    </tr>
                @empty
                    <tr>
                        <th>Sin registros</th>
